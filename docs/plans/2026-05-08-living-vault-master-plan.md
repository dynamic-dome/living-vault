...
| 10b | (Phase 2) Nachbar-Gespräche Stufe 3 (Multi-Persona-Roundtable) | Multi-Select Pages + 3 Modi (Round-Robin / Moderator / Free-for-all) + Cross-Persona-Consult + Geteilte History + UI-Bubbles mit Persona-Label | ✅ |
| 11 | (Phase 2) synesthesia public subset + standalone Embed | Allowlist-Layer, Public-Build CLI, Brand-Header, Deploy-Skript | ✅ |
| 12 | (Phase 2) séance MCP-Tool + commit_insight | seance.summon, seance.commit_insight, Multi-Page-Modus | ✅ |
| 13 | (Phase 2) Version-History in synesthesia-public Vault | core/history (TTL-LRU) + MCP-Tool + history.json + 3D-Panel + CLI | ✅ |
| 14 | Abschluss-Synthese | [[wiki/synthesis/2026-05-10-living-vault-retro]] + [[wiki/entities/living-vault]] (status: done) + log.md | ✅ |
...
Aktuelle Position: **Phasen 8 + 9 + 10a + 10b + 11 + 12 + 13 ✅ ALLE abgeschlossen am 2026-05-10**. **272 Tests grün** (Phase 12 endete bei 253, +19 für Phase 13). Phase 13 baut Git-History-Lookup als 4-Layer-Stack: (1) `core/history.py` mit TTL-LRU-Cache 60s, walked von vault_root nach oben bis zum nächsten `.git`-Verzeichnis, ASCII-Unit-Separator (0x1F) im git-log-format, defensive Fallbacks (kein Git → leere Liste statt Crash); (2) `vault-engine-mcp.page_history(path, limit=10)` als MCP-Tool-Adapter; (3) `synesthesia-public-build` schreibt `out-vault/history.json` (schema v1) und bumpt `manifest.json` auf `schema_version=2` mit `history_included: bool` (additive); (4) `vault-3d.html.j2` zeigt Hover-driven History-Panel rechts unten (lazy fetch, max 10 Commits, CSS-only). CLI: `living-vault history <path> --vault <root> [--limit N]`. **Wichtig:** Original-Master-Plan-Eintrag sagte "Version-History-Modal in **living-portfolio**", User-Entscheidung 2026-05-10 verschob das Modal in den Phase-11-**synesthesia-public Vault** (`vault.dynamic-dome.com`) — konsistenterer Ort. living-portfolio behält nur Phase-1-Funktionalität (Auto-Sync + /now + Freshness). **Phase 14 ✅ abgeschlossen 2026-05-10 — Projekt offiziell fertig.** Retro-Synthese: `~/wiki/wiki/synthesis/2026-05-10-living-vault-retro.md` mit 3-Schichten-These (mechanisch/semantisch/persona-haft), Supporting + Counter-Evidence, übertragbaren Lessons (Master-Plan-Muster, AskUserQuestion vor Spec, Subagent-driven-development pro Sub-Task, Refactor-vor-Adapter, defensive Fallbacks > Exceptions). Wiki-Entity `~/wiki/wiki/entities/living-vault.md` neu angelegt mit status: done und Verweis auf Retro. Log-Eintrag in `~/wiki/log.md`, Index-Update in `~/wiki/index.md`. Naechste Schritte sind keine neuen Phasen, sondern inkrementelle Carry-Over-Items (DNS-Setup, Sichtpruefung gegen Real-Wiki, Allowlist-Bridge-Pages, optional Belief-Evolution als 4. Konsument). Verbleibende offene Items: Phase-9-content-hash-cache-invalidation (Wiki-TODO), Codex-LOW SQLite-IN-Limit (Phase-11-Carry-Over), Allowlist-Bridge-Pages für mehr Edge-Density (organisch), DNS+Hosting-Setup für `vault.dynamic-dome.com` (User-Action), optionaler Wiki-Export von Insights (Phase 13+), `--follow`-Support für Renames in core/history (Phase 13.x).
