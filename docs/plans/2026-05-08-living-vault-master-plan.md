...
Aktuelle Position: **Phasen 8 + 9 + 10a + 10b ✅ und Phase 11 🟡 IN REVIEW (Code komplett, wartet auf User-Sichtprüfung der Live-DB-Smoke)**. **218 Tests grün** (Phase 10b endete bei 204, +14 für Phase 11.1-11.4). Phase 11 entscheidet sich gegen Embed in CV-Site/dome-dynamics, baut stattdessen Standalone-Subroute `vault.dynamic-dome.com`. Public-Subset = Frontmatter `public: true` UNION `docs/public-allowlist.txt`. Neue CLI `synesthesia-public-build` produziert `out-vault/` mit `index.html` + `manifest.json` + `pages.json`. Brand-Header im 3D-Template zeigt Subroute + Page-Count + Build-Date. Deploy-Skript `scripts/deploy-public-vault.ps1` und `docs/DEPLOY-PUBLIC-VAULT.md` erklären 3 Hosting-Pfade (Cloudflare Pages / Netlify / GitHub Pages). DNS-Setup bleibt User-Action. Acceptance-Checklist `docs/PHASE-11-CHECKLIST.md` mit 4 Sichtprüfungs-Stages. **Nach positiver User-Sichtprüfung: Phase 11 ⏳→✅, dann Phase 12 = séance MCP-Tool + commit_insight.** Verbleibende offene Items: Phase-9-content-hash-cache-invalidation (separates Wiki-TODO), Codex-LOW SQLite-IN-Limit (Phase-11-Carry-Over).
