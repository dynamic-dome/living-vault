...
Aktuelle Position: **Phasen 8 + 9 + 10a + 10b + 11 + 12 ✅ ALLE abgeschlossen**. **253 Tests grün** (Phase 11 endete bei 218, +35 für Phase 12). Phase 12 baut den séance-MCP-Server (`seance-mcp` Entry-Point) als zweiten MCP-Layer neben `vault-engine-mcp`. 5 Tools: `summon`, `say`, `commit_insight`, `list_insights`, `list_sessions`. Architektur: Refactor in 12.2 extrahiert die transport-neutrale Orchestrierung aus `apps/seance_ui/app.py` in `apps/seance_ui/orchestrator.py` — FastAPI-UI und MCP-Server greifen beide darauf zu, keine Code-Duplikation. `SéanceError(code, detail)` mappt 1:1 auf HTTP-Status (400/404/410/413/502) bzw. MCP-RuntimeError. Insights-Persistenz via neuer `insights`-Tabelle in `.vault-engine.db` mit additive Migration (idempotent). `session_id` ist nullable — Standalone-Insights ohne aktive Session möglich. Multi-Page-Roundtable funktioniert über MCP genauso wie über UI (1-8 Personas, modes single/roundrobin/moderator/freeforall). Insight-Cap 16k chars als Cost-Guard. **Nächste Phase: 13 = Version-History-Modal in living-portfolio.** Verbleibende offene Items: Phase-9-content-hash-cache-invalidation (separates Wiki-TODO), Codex-LOW SQLite-IN-Limit (Phase-11-Carry-Over), Allowlist-Bridge-Pages für mehr Edge-Density (organischer Prozess), DNS+Hosting-Setup für `vault.dynamic-dome.com` (User-Action), optionaler Wiki-Export von Insights (Phase 12.x oder Phase 13+).
